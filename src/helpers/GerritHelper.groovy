...
class GerritHelper {
    ...
    def gerritSubmit() {
        try {
            ...
            context.println("${gerritSSH()} review --submit ${context.params.GERRIT_PATCHSET_REVISION}")
        } catch (RetriesExceededException e) {
            context.println e
            throw new Exception(e)
        }
    ...
    def getChangedPaths() {
        ...
        try {
            (jsonText, err, exitValue) = context.println(changeQuery)
        } catch (RetriesExceededException e) {
            context.println e
    ...
    def reVerify(refSpec) {
        ...
        if (jobsToRun.size() > 0) {
            ...
            context.println("${gerritSSH()} review --message '\"${warning}\"' "
                                    + context.params.GERRIT_PATCHSET_REVISION)
        }
